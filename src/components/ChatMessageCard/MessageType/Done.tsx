...
const MessageTypeOfDoneCard: React.FC<Props> = ({ item }) => {
    ...
    return (
        <Wrapper isOwn={item.senderId === user?.id}>
            ...
            <Container isOwn={item.senderId === user?.id}>
                <Header>
                    <Typography.H50B color={theme.color.N0}>서명완료</Typography.H50B>
                </Header>
                <Body>
                    <Typography.P100>
                        서명완료<br />
                        {text.sellerName}, {text.goodsTitle}에 대한 
                        의약품 거래내역서의 모든 서명이 완료되었습니다. 문서 내용을 확인해 보세요.<br /><br />

                        ▷문서 이름 :<br />
                        {text.goodsTitle} 의약품 거래내역서<br /><br />
                    ...
                    <FillMediumButton
                        ...
                        disabled={false}
                        handleOnClick={async () => {
                            const res = await goodsPurchaseDetailsMiddleware.getUserReceiptId(text.idForButton);
...
`
export default MessageTypeOfDoneCard;