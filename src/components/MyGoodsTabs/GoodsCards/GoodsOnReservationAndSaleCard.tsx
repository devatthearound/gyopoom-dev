...
const GoodsOnReservationAndSaleCard: React.FC<Props> = ({ goods, refeching, showPopUp }) => {
    ...
    const hanlderOnClick = async (stateToBeChanged: string) => {
        if (stateToBeChanged === GoodsSateCode.RESERVATION) setModalMessage(GoodsSateText.RESERVATION + "로 거래상태를 변경합니다");
        if (stateToBeChanged === GoodsSateCode.SALE) setModalMessage(GoodsSateText.SALE + "로 거래상태를 변경합니다");
        if (stateToBeChanged === GoodsSateCode.SOLDOUT) setModalMessage(GoodsSateText.SOLDOUT + "로 거래상태를 변경합니다");
        if (stateToBeChanged === GoodsSateCode.SUSPEND) setModalMessage(GoodsSateText.SUSPEND + "로 거래상태를 변경합니다");
    ...
    return (
        <CardWrapper>
            ...
            <Flex style={{
                ...
                {goods.goodsStateCode == GoodsSateCode.SALE ?
                    <CardBottom
                        label={GoodsSateText.RESERVATION}
                        onCilckAction={() => hanlderOnClick(GoodsSateCode.RESERVATION)}
                        icon={personPin} />
                    :
                    <CardBottom
                        label={GoodsSateText.SALE}
                        onCilckAction={() => hanlderOnClick(GoodsSateCode.SALE)}
                        icon={personPin} />
                }
                <CardBottom
                    label={GoodsSateText.SOLDOUT}
                    onCilckAction={() => hanlderOnClick(GoodsSateCode.SOLDOUT)}
                    icon={personPin} />
            </Flex>
        </CardWrapper>
    )
...
export default GoodsOnReservationAndSaleCard