...
import { GoodsPurchaseState, GoodsSateCode, GoodsSateText } from "@utils/common-status-code"
...
const MessageTypeOfGoodsCard: React.FC<Props> = ({ goodsId, senderId }) => {
    const { user } = useGetUser()
    const navigate = useNavigate();
    const goodsMiddleware = new GoodsMiddleware();

    const getGoods = async () => {
        const apiRes = await goodsMiddleware.getChatGoodsById(goodsId);
        if (isSuccess(apiRes)) return { ...apiRes.data };
    };

    const { isLoading, data } = useQuery(['goodsOne', goodsId], getGoods);
    ...
    const handleOnClick = () => {
        if (data) {
            ...
            setLocalStorage("buyerId", senderId)
            setLocalStorage("currentRoute", pathname)
            navigate(`/new-purchase-details`)
        }
    }

    const returnAuthorCard = (type: string | null) => {
        switch (type) {
            case null:
                return (
    ...
    const returnDefaultCard = (type: string | null) => {
        switch (type) {
    ...
    return (
        <Wrapper>
            ...
            {
                data.authorId == user?.id ? returnAuthorCard(data.purchasingState)
                    : returnDefaultCard(data.goodsStateCode)
            }
        </Wrapper>
    )
}
